...
public class Scanner {
	...
	public Scanner(String scanURL, String apiKey, String source, String exclude, String rule,
			boolean isPrivateScan, int timeout, boolean isCreateLog, String logFilePath) {
	...
	public ArrayList<ScanResult> call() {
		ArrayList<File> filesToScan = Utils.createFileList(source, exclude);
	...
	private class ScanAction implements Callable<ScanResult> {
		 ...
		 * @return Scan result object with Data ID, an empty data id means file failed
		 *         to scan
		 */
		private ScanResult uploadFile(String filePath, CloseableHttpClient httpClient) {
			...
			try {
				...
				if (response.getStatusLine().getStatusCode() == 200) {
					...
					sc.setDataID("");
					sc.setBlockedReason("HTTP response code: "
			...
			try {
				...
				while (progressPercent < 100 && count <= timeout) {
					...
					if (response.getStatusLine().getStatusCode() == 200) {
						...
						String errMsg = "Polling dataID " + dataID + " returns http code "
								+ response.getStatusLine().getStatusCode();
						Utils.writeLogFile(logFilePath, errMsg + "\n", true, isCreateLog);
					}
				}
